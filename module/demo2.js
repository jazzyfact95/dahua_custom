...
$(function () {
  ...
  WebVideoCtrl.checkPluginInstall()
    .done(function () {
      WebVideoCtrl.insertPluginObject('divPlugin2', 1, 600);
      //초기화 플러그인
      WebVideoCtrl.initPlugin('Protocol2', function () {
        WebVideoCtrl.setOpInfoCallback(showOPInfo);
        var left = $('#divPlugin2').offset().left;
        var top =
          $('#divPlugin2').offset().top +
          window.outerHeight -
          window.innerHeight;
        var width = $('#divPlugin2').width();
        var height = $('#divPlugin2').height();
        WebVideoCtrl.resizeVideo(left, top, width, height);
        //비디오 창을 만듭니다
        WebVideoCtrl.createMultiNodeDisplay(4);
        //비디오 창의 디스플레이를 설정합니다
...
const hyeonseok_play_videos = () => {
  //현재 선택된 장치 IP를 가져옵니다
  // var sIP = $('#ips').find('option:selected').text();

  var sIP = 'hhh4-1.iptime.org';
  //채널 번호를 가져옵니다
  var iChannel = $('#channels').val() - 0;
  var iStreamType = parseInt($('#streamtype').val(), 10);
  var iMode = parseInt($('#winMode').val(), 10);
...
window.onscroll = function () {
  var coverInfo = document.getElementById('divPlugin2').getBoundingClientRect();
