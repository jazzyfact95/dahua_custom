var WebVideoCtrl = (function (e) {
  ...
  var MethodTable = {
    StartDevciceDetection: ['ip', 'port'], //오픈 장치 활성 등록 정보
    CreatePluginWindow: ['browser'], //창, WebSocket 솔루션을 만듭니다
    ResizeVideo: ['left', 'top', 'width', 'height'], //변경 창 위치 크기, WebSocket.
    ShowWindow: ['show'], //창이 표시되는지 여부에 관계없이 WebSocket 솔루션이 사용됩니다.
    SetProductType: ['type'], //제품 유형 설정
    SetSplitNum: ['num'], //부서의 수를 설정하십시오
    GetLastError: ['svrName'], //오류 코드 가져 오기
    GetChannelTotal: ['deviceID'], //장비 채널 수를 얻으십시오
    GetSelectedNodeIndex: [], //현재 선택된 노드 색인을 가져옵니다
    GetSelectedViewIndex: null, //현재 선택된보기 인덱스를 가져옵니다
    GetSelectedWinID: [], //현재 선택된 창 색인을 가져옵니다
    ConnectRealVideo: [
      'deviceID',
      'nodeIndex',
      'viewIndex',
    ...
    CreateMultiNodeDisplay: ['num', 'viewConfig'], //지정된 숫자 만들기 여러 채널보기보기 ViewConfig 기본 에어 문자열보기
    SetFullscreen: null, //전체 화면 디스플레이
    ExitFullscreen: null, //전체 화면을 종료하십시오
    ClosePlayer: ['playerID'], //지정된 플레이어 팀의 반환 값을 ConnectRealTimeVideo로 끄기
    LoginDevice: [
      'ip',
      'svrPort',
      'userName',
      ...
      'specCap',
      'timeout',
    ], //로그인 프로토콜 프런트 엔드는 DahuA3 SVRPort 로그인 포트 SPECCAP 0 : TCP 로그인 모드 2 : 능동적 인 등록 로그인 모드 3 : 멀티 캐스트 로그인 4 : UDP 로그인 모드
    ...
    ResumePlayBack: null, //재생을 복구하십시오
    FastPlayBack: null, //표현하다
    SlowPlayBack: null, //느린
    PlayOneFrame: null, //단일 프레임
    SelectDirectory: ['type'], //로컬 경로 선택 상자 호출 선택한 경로 이름을 가져올 로컬 경로를 선택하십시오.
    StartIntercom: ['deviceID'], //오픈 인터컴
    StopIntercom: ['deviceID'], //확대
    CrabOnePicture: ['format', 'path', 'open'], //파악
    ControlRecordingVideo: ['path', 'format', 'enable'], //동영상
    SetVolume: ['volume'], //볼륨 설정
    ControlAudio: ['playerID', 'enable'], //오디오 스위치
    MoveUpperLeft: ['verticalSpeed', 'levelSpeed', 'flag'], //모바일이 왼쪽으로 이동합니다
    MoveUpperRight: ['verticalSpeed', 'levelSpeed', 'flag'], //Yuntai는 오른쪽으로 움직입니다
    MoveLowerLeft: ['verticalSpeed', 'levelSpeed', 'flag'], //Punxai가 왼쪽으로 옮겼습니다
    ...
    MoveLower: ['verticalSpeed', 'flag'], //아래에
    ...
    RemovePreset: ['index'], //사전 설정 포인트 삭제
    SetPreset: ['index', 'name'], //설정 프리셋 포인트
    ...
    SetLensInfo: ['leninfo'], //렌즈 매개 변수 설정 (더 많은 접합 필요)
    SubscribeEvent: ['channel', 'event'], //구독 행사
    UnSubscribeEvent: ['channel', 'event'], //구독 이벤트를 취소하십시오
    //recordType비디오 유형 0 : 모든 녹음, 1 : 외부 알람, 2 : 동적 테스트 알람, 3 : 모든 알람,
    ...
    OpenVoiceTalk: ['deviceID', 'sampleRate', 'depth', 'encode'], //지정된 매개 변수가 열렸습니다
    CloseVoiceTalk: ['deviceID'], //확대
    GetDeviceConfig: ['deviceID', 'name'], //장비 구성 가져 오기
    StartTour: ['index'], //열린 크루즈
    StopTour: ['index'], //크루즈를 중지하십시오
    EnableCheck: ['bFlag'], //용량
    SetSplitRowAndCol: ['row', 'col'], //분할 수를 설정하십시오
    ...
    StartTourByChannel: ['channel', 'index'], //채널로 크루즈를 타십시오
  };
   ...
   *@param{left}  left 플러그인 영역의 왼쪽 상단 모서리의 위치 (브라우저의 왼쪽 상단 모서리 포함)
   *@param{top}  top 플러그인 영역의 왼쪽 상단 모서리의 위치 (브라우저의 왼쪽 상단 모서리 포함)
   *@param{width}  width플러그인 영역 과이드
   ...
   *@param{Num} iPlayerType  플레이어 유형 0 : 실시간 모니터링 1 : 네트워크 재생
   */
            ...
            if (iRet > 0) {
              ...
              if (typeof fnSuccess != 'undefined') {
                ...
                pluginObject.GetSelectedWinID().done(function (iWinID) {
                  console.log('1번째 iWinID > ', iWinID + 2);
                  insertPlayer(
                    iWinID + 2,
                    ODeviceInfo.deviceID,
                    iRet,
  ...
  /**
   *@description 현재 선택한 창의 플레이어를 닫습니다
   */
  ...
  /**
   *@description 제어
   *@param{Num} iSpeed     다수의
   *@param{Num} flag      로고를 늘리거나 줄입니다
   *       -0 : 증가
   * - 1 : 감소
